...
function optionsframework_add_admin() {
    ...
    $of_page = add_submenu_page('themes.php', THEMENAME, 'Stonex Options', 'edit_theme_options', 'optionsframework','optionsframework_options_page'); // Default
...
<div class="wrap" id="of_container">
  ...
  <form id="of_form" method="post" action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" enctype="multipart/form-data" >
	...
	<button id ="of_save" type="button" class="button-primary"><?php _e('Save Changes', 'framework');?></button>
	</div><!--.info_bar--> 	
	...
	<button id ="of_save" type="submit" class="button-primary"><?php _e('Save Changes', 'framework');?></button>
	<button id ="of_reset" type="submit" class="submit-button reset-button" ><?php _e('Options Reset', 'framework');?></button>
	</div><!--.save_bar--> 
...
function of_style_only(){
	wp_enqueue_style('admin-style', ADMIN_DIR . 'admin-styles.css');
...
function of_load_only() {
	...
	wp_enqueue_script('ibutton', ADMIN_DIR.'js/jquery.ibutton.js', array('jquery'));
	wp_enqueue_script('jquery-tools-min', ADMIN_DIR.'js/jquery.tools.min.js', array('jquery'));
	wp_enqueue_script('cookie', ADMIN_DIR . '/js/cookie.js', 'jquery');
	wp_enqueue_script('jquery-tools-min',  '/js/cookie.js', 'jquery');
...
function of_admin_head() { 
	...
	jQuery(":range").rangeinput({progress: true, keyboard: true});
	//hide hidden section on page load.
	jQuery('#section-body_bg, #section-body_bg_custom, #section-body_bg_properties').hide();
	
	// custom js for home slider select box upon change
jQuery('#slider_type').change(function() {
    if (jQuery('#slider_type').val() == "flex_slider") {
        jQuery('#section-featured_slider, #section-slider_effect, #section-slider_speed, #section-slider_pause_time, #section-slider_pause').show();
        jQuery('#section-static_image').hide();
    } else {
    if (jQuery('#slider_type').val() == "static_image") {
        jQuery('#section-static_image').show();
        jQuery('#section-featured_slider, #section-slider_effect, #section-slider_speed, #section-slider_pause_time, #section-slider_pause').hide();
            }
        }
});
...
//show and hide clients logo section based of the currently selected option

jQuery('#our_clients').change(function() {
    if (jQuery('#our_clients').is(':checked')) {
        jQuery('#section-client_logo_title, #section-client_logo1, #section-client_logo2, #section-client_logo3, #section-client_logo4, #section-client_logo5, #section-client_logo6').show();
    } else {
		jQuery('#section-client_logo_title, #section-client_logo1, #section-client_logo2, #section-client_logo3, #section-client_logo4, #section-client_logo5, #section-client_logo6').hide();
        }
});
...
//show and hide intro section based of the currently selected option

jQuery('#intro_hide').change(function() {
    if (jQuery('#intro_hide').is(':checked')) {
        jQuery('#section-intro_content').show();
    } else {
		jQuery('#section-intro_content').hide();
        }
});
...
//show and hide custom footer copyright section based of the currently selected option

jQuery('#footer_copyright').change(function() {
    if (jQuery('#footer_copyright').is(':checked')) {
        jQuery('#section-footer_copyright_text').show();
    } else {
		jQuery('#section-footer_copyright_text').hide();
        }
});
...
//show and hide custom header text section based of the currently selected option

jQuery('#custom_text_hide').change(function() {
    if (jQuery('#custom_text_hide').is(':checked')) {
        jQuery('#section-custom_text_content').show();
    } else {
		jQuery('#section-custom_text_content').hide();
        }
});
...
if (jQuery('#custom_text_hide').is(':checked')) {
	...
	styleSelect = {
		init: function () {
		$('.select_wrapper').each(function (asd) {
		...
		$('.typography-face').each(function (asd) {
			$(this).append('<p class="' + $(this).find('.select option:selected').val() + '">' + $(this).find('.select option:selected').text() + '</p>');
		});
		$('.select').live('change', function () {
			var font = $(this).find('option:selected').val()
			$(this).next('p').replaceWith('<p class="'+ font +'">' + $(this).find('option:selected').text() + '</p>');
		});
		$('.select').bind($.browser.msie ? 'click' : 'change', function(event) {
			var font = $(this).find('option:selected').val()
			$(this).next('p').replaceWith('<p class="'+ font +'">' + $(this).find('option:selected').text() + '</p>');
	...
	// Sort Slides
	jQuery('.slider1').find('ul').each( function() {
	...
	$('#of_backup_button').live('click', function(){
		...
		var answer = confirm("<?php _e('Click OK to backup your current saved options.', 'framework');?>")
	...
	$('#of_restore_button').live('click', function(){
		...
		var answer = confirm("<?php _e('Warning: All of your current options will be replaced with the data from your last backup! Proceed?', 'framework');?>")
	...
	$('#of_reset').click(function() {
		var answer = confirm("<?php _e('Click OK to reset. All settings will be lost!', 'framework');?>")
...
public static function optionsframework_machine($options) {
	...
	foreach ($options as $value) {
		...
		case 'range':
			...
			if ( $std != "") { $val = $std; }
			$output .= '<input class="" name="'. $value['id'] .'" id="'. $value['id'] .'" min="'. $value['min'] .'" max="'. $value['max'] .'" type="'. $value['type'] .'" value="'. $val .'" />';
		break;
		case 'select':
			$mini ='';
			if(!isset($value['mod'])) $value['mod'] = '';
		...
		case 'checkbox':
			...
			$output .= '<input type="hidden" class="checkbox aq-input" name="'.$value['id'].'"  value="0"/>';
		...
		case 'typography':
			...
			if(isset($typography_stored['face'])) {
				...
				$output .= '<select class="of-typography of-typography-face select muc" name="'.$value['id'].'[face]" id="'. $value['id'].'_face">';
		...
		case 'images':
			...
			$i++;
				...
				$output .= '<img src="'.$option.'" alt="" class="of-radio-img-img '. $selected .' bg-' . $value['id'] . $i . '" onClick="document.getElementById(\'of-radio-img-'. $value['id'] . $i.'\').checked = true;" />';
				$output .= '</span>';				
			}
			
		break;	
		case "info":
			$file_path = get_stylesheet_directory() . '/styles/custom_options.css';;

			if (is_writeable($file_path)) {
			$persmission = '';
			} else {
			$persmission = '<p class="permission"><strong>NOTE: The file "custom_options.css" located in the theme\'s folder > styles does not have write permissions. Make sure the file is writable before your custom styling can be saved. If you don\'t know how to set file permissions <a href="#">click here</a></strong></p>';
		}
			$info_text = $value['std'];
			$output .= '<div class="of-info">'.$info_text.$persmission.'</div>';
		break;
		case 'heading':
			if($counter >= 2){
			   $output .= '</div>'."\n";
		...
		case 'slider':
			...
			$int = '';
			$int = optionsframework_mlu_get_silentpost( $_id );
			$output .= '<div class="slider1"><ul id="'.$value['id'].'" rel="'.$int.'">';
